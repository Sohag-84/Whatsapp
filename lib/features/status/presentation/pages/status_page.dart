...
import 'package:whatsapp/features/status/presentation/widgets/status_dotted_border_widget.dart';
...
class _StatusPageState extends State<StatusPage> {
  ...
  void initState() {
    ...
    context.read<GetMyStatusCubit>().getMyStatus(uid: widget.currentUser.uid!);

    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      di.sl<GetMyStatusFutureUsecase>().call(uid: widget.currentUser.uid!).then(
        (myStatus) {
      ...
      builder: (context, state) {
        ...
        if (state is StatusLoaded) {
          ...
          return BlocBuilder<GetMyStatusCubit, GetMyStatusState>(
            builder: (context, state) {
              if (state is GetMyStatusLoading) {
                return loader();
              }
              if (state is GetMyStatusFailure) {
                return Center(child: Text(state.error));
              }
                ...
                .then((value) {
                  Navigator.pushReplacement(
                    ...
                    MaterialPageRoute(
                      builder:
                          (_) =>
                              HomePage(uid: widget.currentUser.uid!, index: 1),
                    ),
                  );
                });
          } else {
